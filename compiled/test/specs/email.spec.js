...
define('specs/email', ['mocks/hoodie', 'email'], function(CangMock, Email) {
  return describe("Email", function() {
    ...
    return describe(".send(email_attributes)", function() {
      beforeEach(function() {
        ...
        return (spyOn(this.app.store, "create")).andReturn({
          then: function(cb) {
      ...
      it("should save the email as object with type: $email", function() {
        ...
        return (expect(this.app.store.create)).wasCalledWith('$email', this.email_attributes);
      });
      it("should listen to server response", function() {
        spyOn(this.app, "one");
        this.email.send(this.email_attributes);
        (expect(this.app.one)).wasCalled();
        return (expect(this.app.one.mostRecentCall.args[0])).toEqual("remote:updated:$email:abc4567");
      });
      ...
      _when("sending email was successful", function() {
        beforeEach(function() {
          ...
          (spyOn(this.app, "one")).andCallFake(function(event, cb) {
