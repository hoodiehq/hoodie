...
test('db.grantPublicWriteAccess / revokePublicWriteAccess', function (t) {
  ...
  hoodie.database.add('foo', function (err, db) {
    ...
    async.series(tasks.map(ignoreErrs), function (err, results) {
      ...
      t.equal(results[7][0].statusCode, 201) // testuser2 write
      t.equal(results[8][0].statusCode, 200) // anonyous read
      t.equal(results[9][0].statusCode, 201) // anonymous write
      ...
      t.equal(results[13][0].statusCode, 200) // testuser2 read
      t.equal(results[14][0].statusCode, 401) // testuesr2 write
      t.equal(results[15][0].statusCode, 200) // anonymous read
