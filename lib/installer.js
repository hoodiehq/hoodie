...
exports.setupModules = function (config, callback) {
    // setup modules DB
    npm.load(function (err, npm) {
        if (err) {
            return callback(err);
        ...
        var password = (
            npm.config.get(config.app.name + '_admin_pass') ||
...
exports.createModulesDB = function (config, password, callback) {
    request({
        url: config.couch.url + '/modules',
        method: 'PUT',
        auth: {
            user: 'admin',
            pass: password
...
exports.createAppConfig = function (config, password, callback) {
    try {
    ...
    request({
        url: config.couch.url + '/modules/module%2Fappconfig',
        method: 'PUT',
        auth: {
            user: 'admin',
            pass: password
