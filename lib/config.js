...
module.exports = function (options) {
  ...
  if (argv.hasOwnProperty('custom-ports')) {
    ...
    log.verbose('config', 'Using custom ports ' + c_ports.join(', '))
    ...
    log.warn('config', 'No App port specified. Will be available at ' + env_config.host + ':' + env_config.www_port)
    env_config.admin_port = ports.getPort(env_config.id + '-admin')
    log.warn('config', 'No Admin port specified. Will be available at ' + env_config.host + ':' + env_config.admin_port)
  }
  ...
  if (!env.COUCH_URL && !argv.hasOwnProperty('custom-ports')) {
    ...
    log.warn('config', 'No database port specified. Will be available at ' + env_config.host + ':' + env_config.couch.port)
  }

  if (env.HOODIE_SETUP_PASSWORD) {
    log.info('config', 'Reading admin password from environment.')
