...
function getPlugins (env_config) {
  return Object.keys(env_config.plugins)
    .map(function (id) {
      var name = id.replace(/^hoodie-plugin-/, '')
      return path.resolve(
        env_config.plugins[id].path, 'hoodie.' + name + '.js'
      )
    })
...
module.exports = function (env_config, done) {
  // If we have a valid value in `cache` we can simply return that.
  if (cache) {
    log.silly('Serving browserified frontend bundle from cache')
    return done(cache)
  }
  ...
  if (env_config.app.hoodie && env_config.app.hoodie.hoodiejs) {
    hoodiejs = path.join(env_config.project_dir, env_config.app.hoodie.hoodiejs)
  }
  ...
  var plugins = getPlugins(env_config)
  ...
  b.bundle(function (err, bundle) {
    ...
    fs.unlinkSync(bundleConcat)
    log.verbose('Browserifying frontend bundle')
    done(bundle)
  })
}
