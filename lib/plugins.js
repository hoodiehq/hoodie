...
exports.getPluginPaths = function (cfg) {
    ...
    if (!app.hoodie.plugins) {
        // no plugins property, infer plugins from dependencies
        var names = Object.keys(app.dependencies).filter(function (d) {
            return /^hoodie-plugin-/.test(d);
        });
        app.hoodie.plugins = names.map(function (name) {
            return './node_modules/' + name;
        });
    }
    return app.hoodie.plugins.map(function (p) {
        return path.resolve(cfg.project_dir, p);
    });
};
