...
exports.getConfig = function (platform, env, project_dir, argv) {
  ...
  if (argv.hasOwnProperty('custom_ports')) {
    var c_ports = argv['custom_ports'].split(',').reduce(function (memo, port) {
    ...
    if (c_ports.length === 3) {
      cfg.www_port = c_ports[0];
      cfg.admin_port = c_ports[1];
      cfg.couch.port = c_ports[2];
    } else {
  ...
  if (!process.env.COUCH_URL && !argv.hasOwnProperty('custom_ports')) {
