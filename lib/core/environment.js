...
exports.getConfig = function (platform, env, project_dir, argv) {
  ...
  var cfg = {
    ...
    id: pkgfile.name,
    system_id: pkgfile.name + '-' + crc32(project_dir),
  ...
  cfg.www_port = ports.getPort(cfg.system_id + '-www');
  cfg.www_link = function () {
    return cfg.local_tld
      ? cfg.www_local_url
      : 'http://' + cfg.host + ':' + cfg.www_port;
  };
  cfg.admin_port = ports.getPort(cfg.system_id + '-admin');
  ...
  if (!process.env.COUCH_URL) {
    cfg.couch.port = ports.getPort(cfg.system_id + '-couch');
  }
