...
exports.path = function (env_config) {
  return path.resolve(env_config.hoodie.app_path, 'config.json');
};
...
exports.read = function (env_config, callback) {
  var obj;
  var env_configPath = exports.path(env_config);

  console.log('Reading config from %s.', env_configPath);

  fs.readFile(env_configPath, function (err, data) {
    if (err) {
