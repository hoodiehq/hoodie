...
var hoodie_server = function(req, res, proxy) {
  ...
  if(this.serve_static(host, this.name)) {
    console.log("[static req] %s", req.url);
  ...
  if(this.serve_admin(host, this.name)) {
    console.log("[admin req] %s", req.url);
    admin_server = static_http("./node_modules/hoodie-app/node_modules/hoodie-pocket/dist");
