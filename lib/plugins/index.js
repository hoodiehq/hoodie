...
exports.load = function (env_config, callback) {
  ...
  env_config.plugins = _(env_config.plugins)
  .map(function (plugin) {
    var pluginPath = exports.resolvePluginPath(env_config.paths.project, plugin)
    var pkg = JSON.parse(fs.readFileSync(path.join(pluginPath, 'package.json')))
    var hapiHook = exports.loadModule(path.join(pluginPath, 'hooks', 'hapi'))
    var staticHook = exports.loadModule(path.join(pluginPath, 'hooks', 'static'))
