...
exports.load = function (env_config, callback) {
  ...
  .reduce(function (acc, p) {
    var pkg = JSON.parse(fs.readFileSync(path.resolve(p, 'package.json')))
    acc[pkg.name] = {
      path: p,
      pkg: pkg,
      hooks: {
        static: exports.readStaticPluginHook(p, pkg.name)
      }
    }
    return acc
  }, {})
...
exports.startPlugin = function (name, env_config, hoodie, callback) {
  ...
  var doStartPlugin = function (p, cb) {
    var backendModule = exports.loadModule(path.join(p, 'backend'))
    if (!backendModule) {
      log.silly('plugins', 'No backend module found for ' + name)
      return cb()
    }
    log.silly('plugins', 'Backend module found for ' + name)
    return backendModule(hoodie, cb)
  }
  ...
  var doInitHooks = function (p, cb) {
    ...
    var pluginEE = new EventEmitter()
    pluginEE._name = name
    env_config.plugins[name].hooks.dynamic = pluginEE
    dynamicHook(hoodie, pluginEE, cb)
  }
...
exports.readStaticPluginHook = function (p, name) {
  try {
    var registerFn = require(path.join(p, 'hooks', 'static'))
    if (typeof registerFn !== 'function') return null
    registerFn.attributes = registerFn.attributes || {}
    registerFn.attributes.name = name
    return registerFn
  } catch (e) {
    return null
  }
