...
exports.start = function (env_config, callback) {
  ...
  if (env_config.in_memory) {
    log.info('Database is only in memory, so no changes will be persisted onto disk')
    spawnConf.backend = false
  }
  spawnPouchDBServer(spawnConf, function (err) {
    if (err) {
      log.error('Could not start PouchDB Server for you', err)
      return callback(err)
    }

    log.info('PouchDB Server started')
    callback(null)
  })
}
...
exports.checkExternalCouch = function (couch_url, callback) {
  log.info('Using external database ' + couch_url)
  log.silly('Checking if external database is supported')

  request(couch_url, function (err, res, body) {
    if (err || (res && res.statusCode !== 200)) {
      log.error('Could not find CouchDB at ' + couch_url)
      return callback(err)
    }
    ...
    if (vendor === 'express-pouchdb') {
      log.verbose('External database (' + vendor + ') supported')
      return callback(null)
    }

    log.silly('Checking if CouchDB is above 1.2.0')
    var compatible = semver.gte(version, '1.2.0')

    if (compatible) {
      log.verbose('CouchDB is above 1.2.0 (' + version + ')')
      return callback(null)
    }
